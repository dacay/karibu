...
export const organizations = pgTable('organizations', {
  ...
  // opened this way are short-lived (see externalIdSessionHours). Off by default;
  // no admin UI — set directly in SQL.
  externalIdLoginEnabled: boolean('external_id_login_enabled').notNull().default(false),
  // What the org calls that ID, shown as the field label on the access page
  // (e.g. 'UCSF ID'). Null falls back to a generic 'ID'.
  externalIdLabel: text('external_id_label'),
  ...
  externalIdSessionHours: integer('external_id_session_hours').notNull().default(12),
