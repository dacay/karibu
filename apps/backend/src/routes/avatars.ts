...
import { uploadToS3, deleteFromS3, buildAvatarImageKey, getPresignedUrl } from '../services/s3.js';
...
/**
 * Attach a presigned image URL to an avatar record if it has an S3 key.
 */
async function withImageUrl<T extends { imageS3Key: string | null }>(
  avatar: T
  ...
  try {
    ...
    const imageUrl = await getPresignedUrl(avatar.imageS3Key);
...
avatarsRouter.get('/', requireRole('admin'), async (c) => {
  ...
  const result = await Promise.all(rows.map(withImageUrl));

  return c.json({ avatars: result });
});
...
avatarsRouter.post('/', requireRole('admin'), async (c) => {
  ...
  if (imageFile && imageFile instanceof File && imageFile.size > 0) {
    ...
    const s3Key = buildAvatarImageKey(auth.organizationId, avatar.id, imageFile.name);

    try {

      const buffer = Buffer.from(await imageFile.arrayBuffer());
      const { s3Bucket } = await uploadToS3(s3Key, buffer, imageFile.type);
      ...
      const result = await withImageUrl(updated);
  ...
  const result = await withImageUrl(avatar);
  return c.json({ avatar: result }, 201);
});
...
avatarsRouter.patch('/:id', requireRole('admin'), async (c) => {
  ...
  if (imageFile && imageFile instanceof File && imageFile.size > 0) {
    ...
    const s3Key = buildAvatarImageKey(auth.organizationId, id, imageFile.name);
    ...
    try {
      ...
      if (avatar.imageS3Key && avatar.imageS3Key !== s3Key) {
        ...
        await deleteFromS3(avatar.imageS3Key).catch((err) => {
