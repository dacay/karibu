...
2. For each file, a description is matched from the org's `report_descriptions` rules (case-insensitive filename substring; longest match wins).
...
`report_descriptions` table (`src/db/schema.ts`):
- `organization_id` — FK to `organizations` (cascade delete)
- `match_text` — case-insensitive substring matched against the filename
- `description` — the text shown for matching files
...
| `GET` | `/reports/descriptions` | List the org's description rules |
| `POST` | `/reports/descriptions` | Create a rule `{ matchText, description }` |
| `PATCH` | `/reports/descriptions/:id` | Update a rule |
| `DELETE` | `/reports/descriptions/:id` | Delete a rule |
