...
import { useExternalIdLogin } from "@/hooks/useExternalIdLogin";
...
function SingleInviteForm({ onClose }: SingleInviteFormProps) {
  const queryClient = useQueryClient();
  const { enabled: idLoginEnabled, label: idLabel } = useExternalIdLogin();
  ...
  const inviteMutation = useMutation({
    mutationFn: () =>
      api.team.inviteOne({
        ...
        ...(idLoginEnabled ? { externalId: externalId.trim() || null } : {}),
        sendEmail,
      }),
    onSuccess: (data) => {
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMember(null)}>
        <DialogContent className="max-w-md">
            ...
            onSubmit={(e) => {
              ...
              if (editingMember && editPhoneValid && editExternalIdValid) {
                updateMemberMutation.mutate({
                  ...
                  ...(idLoginEnabled ? { externalId: editExternalId.trim() || null } : {}),
                });
              }
            }}
            className="space-y-4"
          >
            ...
            {idLoginEnabled && editingMember?.role === "user" && (
              <div className="space-y-1.5">
                <label className="text-sm text-muted-foreground">{idLabel}</label>
                <Input
                  placeholder={idLabel}
